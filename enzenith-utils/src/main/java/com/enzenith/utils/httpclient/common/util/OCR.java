...
public class OCR {
	...
	public static String ocrCode(String imgFilePath, int limitCodeLen){
		...
		if(!f.exists()){
  		...
  		String html;
		try {
			html = HttpClientUtil.upload(HttpConfig.custom().client(client).url(API_URL).files(new String[]{imgFilePath},"ocrfile",true).map(getParaMap()));
