package com.enzenith.utils.httpclient.common.util;
...
import com.enzenith.utils.httpclient.common.HttpHeader;
import com.enzenith.utils.httpclient.common.Utils;
 ...
 *
 * @ClassName: OldOCR
 * @Description:识别验证码，自拼接http报文信息
 * @author: jikunle
 * @date: 2018年11月1日 下午2:53:26
 */
public class OldOCR {
    ...
    private static final String API_KEY = PropertiesUtil.getProperty("OCR.key");
    ...
    public static String ocrCode(String imgFilePath, int limitCodeLen) {
        ...
        String html;
        try {
            html = HttpClientUtil.post(HttpConfig.custom().client(client).url(API_URL).headers(HEADERS).map(m));
    ...
    public static String ocrCode4Net(String imgUrl, int limitCodeLen) {
        ...
        try {
            String html = HttpClientUtil.post(HttpConfig.custom().client(client).url(API_URL).headers(headers).map(map));
            String[] results = StringUtil.regex("<Result>([^<]*)</Result>\\s*<Status>([^<]*)</Status>", html);
            if (results.length > 0) {
                if (Boolean.parseBoolean(results[1])) {
                    //不判断长度或者长度一致时，直接返回
    ...
    public static String ocrCode4Net(HttpConfig config, String savePath, int limitCodeLen) {
        ...
        String html;
        try {
            html = HttpClientUtil.post(config.client(client).url(API_URL).headers(HEADERS).map(m));
            String[] results = StringUtil.regex("<Result>([^<]*)</Result>\\s*<Status>([^<]*)</Status>", html);
            if (results.length > 0) {
                if (Boolean.parseBoolean(results[1])) {
                    //不判断长度或者长度一致时，直接返回
