...
public class Blowfish {
    ...
    public byte[] encrypt(final String input) {
        try {
            final byte[] bytes = input.getBytes(UtilsConstant.UTF_8);
    ...
    public String decryptString(final byte[] source) {
        try {
            return new String(decryptBytes(source), UtilsConstant.UTF_8);
        } catch (UnsupportedEncodingException e) {
            return null;
        }
    }
    ...
    private void processTable(
        ...
        for (int s = 0; s < size; s += TWO) {
            xl ^= P[0];

            for (int i = 1; i < ROUNDS; i += TWO) {
                xr ^= F(xl) ^ P[i];
                xl ^= F(xr) ^ P[i + 1];
            }
